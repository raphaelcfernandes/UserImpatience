...
class BackgroundService : Service() {
    ...
    private var spotify_log = Vector<Int>()
    private var gmail_log = Vector<Int>()
    ...
    override fun onCreate() {
        ...
        training = Preferences.getTraining(this)
    ...
    private val broadcastRcv = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
			...
			is the app that was complained in, so both are checked.*/
                    ...
                    else if (currentApp == "com.spotify.music" || lastApp == "com.spotify.music")
                    {
                        spotify_log.add(CpuManager.getFreqPosFromCore(0))

                    }
                    else if (currentApp == "com.google.android.apps.photos" || lastApp == "com.google.android.apps.photos")
                    {
                        photos_log.add(CpuManager.getFreqPosFromCore(0))

                    }
                    else if (currentApp == "com.google.android.gm" || lastApp == "com.google.android.gm")
                    {
                        gmail_log.add(CpuManager.getFreqPosFromCore(0))

                    }
                    else if(currentApp == "com.google.android.youtube" || lastApp == "com.google.android.youtube")
                    {
                        youtube_log.add(CpuManager.getFreqPosFromCore(0))
        ...
        try {
            ...
            if(chrome_log.size > 0) {
                ...
                FileOutputStream(log).use {
                    for (i in chrome_log) {
                        log.appendText(i.toString() + "\n")
                    }
                }
            }

            if(spotify_log.size > 0) {

                val name = "spotify.txt"
                val log = File(dir, name)
                FileOutputStream(log).use {
                    for (i in spotify_log) {
                        log.appendText(i.toString() + "\n")
                    }
                }
            }

            if(photos_log.size > 0) {
            ...
            if(youtube_log.size > 0) {

                val name = "youtube.txt"
                val log = File(dir, name)
                FileOutputStream(log).use {
                    for (i in youtube_log) {
                        log.appendText(i.toString() + "\n")
                    }
                }
            }
